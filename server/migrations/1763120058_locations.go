package migrations

import (
	"github.com/pocketbase/pocketbase/core"
	m "github.com/pocketbase/pocketbase/migrations"
	"github.com/pocketbase/pocketbase/tools/types"
)

const LocationsId = "locations"

func init() {
	m.Register(func(app core.App) error {
		users, err := app.FindCollectionByNameOrId(UsersId)
		if err != nil {
			return err
		}

		families, err := app.FindCollectionByNameOrId(FamiliesId)
		if err != nil {
			return err
		}

		locations := core.NewBaseCollection(LocationsId)

		locations.CreateRule = types.Pointer(`@request.auth.id != ""`)
		locations.ViewRule = types.Pointer(`@request.auth.id != "" && (@request.auth.id = user.id || family.members.id ?= @request.auth.id)`)
		locations.UpdateRule = types.Pointer(`@request.auth.id != "" && @request.auth.id = user.id`)

		locations.Fields.Add(&core.GeoPointField{
			Name:     "coordinates",
			Hidden:   true,
			Required: true,
		})

		locations.Fields.Add(&core.RelationField{
			Name:         "family",
			CollectionId: families.Id,
			Presentable:  true,
			Required:     true,
		})

		locations.Fields.Add(&core.RelationField{
			Name:         "user",
			CollectionId: users.Id,
			Presentable:  true,
			Required:     true,
		})

		locations.Fields.Add((&core.AutodateField{
			Name:     "createdAt",
			System:   true,
			OnCreate: true,
		}))

		locations.Fields.Add(&core.AutodateField{
			Name:     "updatedAt",
			System:   true,
			OnCreate: true,
			OnUpdate: true,
		})

		locations.AddIndex("idx_location_family_user", true, "family,user", "")

		return app.Save(locations)
	}, func(app core.App) error {
		locations, err := app.FindCollectionByNameOrId(LocationsId)
		if err != nil {
			return err
		}

		return app.Delete(locations)
	})
}
